...
public class Percolation {
    private boolean[] grid; // holds whether each site is open or not
    private int n; // size of grid
//     private QuickFindUF qf;
    private WeightedQuickUnionUF wqu; // WeightedQuickUnionUF object
    ...
    public Percolation(int n) {
        ...
        grid = new boolean[n * n + 2]; // one extra for the additional top
        // (n*nth element), another for the virtual bottom (n*n+1th)
//        qf = new QuickFindUF(this.n * this.n + 2);
    ...
    public void open(int row, int col) {
        if (!validSite(row, col)) {
            throw new IllegalArgumentException("not a valid site.");
        }
...
//             qf.union(row * this.n + col, (row - 1) * this.n + col);
    ...
    public boolean isFull(int row, int col) {
        ...
        return wqu.find(n * n) == wqu.find(row * n + col); // check if connected
//        return wqu.connected(n * n, row * n + col); connected() is deprecated
//         return qf.connected(n * n, row * n + col);
    ...
    public boolean percolates() {
        return wqu.find(n * n) == wqu.find(n * n + 1); //check if connected
//        return wqu.connected(n * n, n * n + 1); connected() is deprecated
//         return qf.connected(n * n, n * n + 1); // check if additional top
//         and additional bottom connect
    }
